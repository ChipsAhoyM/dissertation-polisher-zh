...
- **Claude Code 版**：仓库根目录
- **Codex 版**：`codex/dissertation-polisher-zh/`
...
1. 下载本项目
2. 将 `codex/dissertation-polisher-zh/` 复制到 Codex skills 目录：
   - **macOS/Linux**: `~/.codex/skills/`
3. 确保文件夹结构如下：
   ```
   ~/.codex/skills/dissertation-polisher-zh/
   ├── SKILL.md
   └── agents/
       └── openai.yaml
   ```
...
```text
Use $dissertation-polisher-zh to 审阅我的博士论文：/path/to/thesis.pdf
```

#### 场景 2：DOCX / LaTeX 输入

```text
Use $dissertation-polisher-zh to 审阅：~/Documents/博士论文终稿.docx
Use $dissertation-polisher-zh to 审阅：~/thesis/main.tex
```

#### 场景 3：直接粘贴文本

```text
Use $dissertation-polisher-zh to 润色以下博士论文摘要：

[粘贴论文文本]
```

Codex 版会优先使用本地 shell 工具读取 PDF/DOCX/LaTeX，最终在输入文件同目录输出 `*_审阅报告.md`；如为文本粘贴模式，则直接在聊天中输出单章审阅结果。
...
- **`SKILL.md`** — Claude Code 版技能定义文件
- **`codex/dissertation-polisher-zh/SKILL.md`** — Codex 版技能定义文件
- **`codex/dissertation-polisher-zh/agents/openai.yaml`** — Codex UI 元数据
- **`README.md`** — 双平台说明文档
